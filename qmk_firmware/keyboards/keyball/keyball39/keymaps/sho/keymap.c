...
#define HK(kc) (0x8000 | (kc))

bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    if (record->event.pressed) {
        if (keycode & 0x8000) {
            uint16_t base_keycode = keycode & 0x7FFF; // 元のキーコードを抽出

            tap_code(KC_LNG2);      // KC_LNG2を送信
            tap_code(base_keycode); // 抽出したキーコードを送信

            return false; // 元のキー送信をスキップ
        }
    }
    return true; // 既存の処理を続行
}
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  SFT_T(KC_Z), KC_X     , KC_C     , KC_V     , KC_B     ,                            KC_N     , KC_M     , KC_COMM  , KC_DOT   ,SFT_T(KC_MINS),
    KC_ESC   , XXXXXXX  , KC_LGUI  ,LT(1,KC_TAB),LT(2,KC_SPC),LT(3,KC_LNG2),ALT_T(KC_LNG1),KC_BSPC,XXXXXXX, XXXXXXX  ,XXXXXXX   , MO(4)
  ),
  ...
  [1] = LAYOUT_universal(
    ...
    XXXXXXX  , XXXXXXX  , XXXXXXX  , XXXXXXX  , XXXXXXX  , XXXXXXX  ,      KC_DEL   , _______  , XXXXXXX  , XXXXXXX  , XXXXXXX  , XXXXXXX
  ),

  [2] = LAYOUT_universal(
    KC_PAST  , KC_P7    , KC_P8    , KC_P9    , KC_PSLS  ,                            XXXXXXX  , KC_HOME  , KC_UP    , KC_END   , XXXXXXX  ,
    KC_NUM   , KC_P4    , KC_P5    , KC_P6    , KC_PEQL  ,                            KC_PGUP  , KC_LEFT  , KC_DOWN  , KC_RGHT  , KC_PGDN  ,
    KC_P0    , KC_P1    , KC_P2    , KC_P3    , KC_PPLS  ,                            XXXXXXX  , XXXXXXX  , KC_COMM  , KC_PDOT  , KC_PMNS  ,
